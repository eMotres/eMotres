...
export default function ZhBlogPage() {
  ...
  const posts = getPosts(category, 'zh');
    ...
    <main>
      <div className="bg-surface-secondary py-16">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <h1 className="text-4xl font-extrabold text-center mb-4 text-text-primary">技术博客</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {posts.length > 0 ? (
              posts.map((post, index) => (
                <BlogPost key={index} {...post} readMoreLabel="继续阅读 →" />
              ))
            ) : (
              <p className="col-span-full text-center text-text-secondary">该分类下暂无文章。</p>
            )}
          </div>
        </div>
      </div>    </main>
